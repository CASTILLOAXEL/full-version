...
const Helpers = {
  ...
  getStoredTheme: themeName => {
    if (window.templateCustomizer && window.templateCustomizer._getSetting) {
      themeName = window.templateCustomizer._getSetting('Theme')
      if (window.templateCustomizer.settings && window.templateCustomizer.settings.defaultTheme) {
        return themeName || window.templateCustomizer.settings.defaultTheme
      }
    } else {
      themeName = document.documentElement.getAttribute('data-bs-theme')
    }
    return themeName || 'light'   // fallback seguro
  },
